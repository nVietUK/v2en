...
RUN apt update && apt upgrade -y && apt install curl ca-certificates -y
# fixed slow apt download
...
RUN apt install gcc portaudio19-dev wget git vim firefox gh build-essential \
...
RUN echo '/bin/scream' >> /home/${name}/.profile
